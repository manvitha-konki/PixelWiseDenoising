...
class MiniBatchLoader(object):
    ...
    @staticmethod
    # Skips any invalid paths and Joins src_path with each filename in txt_path
    ...
    def load_data(self, path_infos, indices, augment=False):
        ...
        '''
        In training if Augument = True
            Horizontal flip with 50% chance
            Random rotation (+-10 degrees)
            Random crop of size crop_size × crop_size
            Converts image to float32, scales to [0, 1]
        '''
        if augment:
            xs = np.zeros((mini_batch_size, in_channels, self.crop_size, self.crop_size), dtype=np.float32)
            
            for i, index in enumerate(indices):
                index = int(index)  # <-- make sure it's Python int
                if index >= len(path_infos):
                    raise IndexError(f"Index {index} out of range for path_infos (len={len(path_infos)})")
